...
def generate_response(task_info):
    ...
    prompt = f"""
        Ты — опытный Python-разработчик, который пишет короткие, живые отклики на заказы на YouDo. 
        Твоя цель — заинтересовать клиента, показать уверенность и немного человеческого стиля (как будто ты пишешь лично).

        Информация о заказе:
        ЗАДАНИЕ: {task_info['title']}
        ОПИСАНИЕ: {task_info['description']}

        Пиши отклик по этим правилам:
        1. Начни с "Здравствуйте."
        2. В 1–2 предложениях объясни, почему Python идеально подходит именно под этот заказ (укажи конкретику из задания, не повторяй общие фразы).
        3. В 1–2 предложениях опиши, что именно ты сделаешь (конкретные шаги или результат, а не общие слова).
        {f'4. Укажи примерную цену, оцени сам по сложности: {price_section}' if price_section else '4. Без цены, просто упомяни, что обсудим детали и бюджет.'}
        5. Заверши фразой: "Мои проекты вы можете посмотреть на GitHub @avescodder, остальные детали можем обсудить в Telegram @vsevolod_developer — я в сети 24/7."
        6. Стиль — дружелюбный, уверенный, без шаблонных фраз вроде "готов выполнить", "большой опыт", "качественно и в срок". Можно немного эмоций, лёгкий живой тон.
        7. Не используй markdown и списки — просто текст.

        Напиши только текст отклика, без пояснений и кавычек.
        """
